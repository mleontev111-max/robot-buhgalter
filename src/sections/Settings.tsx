      ...
      {state.stores.map((store) => (
        <Card key={store.id} className="bg-white">
          ...
          <CardContent className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            {(store.regime === 'usn6' || store.regime === 'usn15') && <div className="space-y-1.5"><Label className="text-xs">Доход за предыдущий год, ₽</Label><Input inputMode="numeric" value={store.priorYearRevenue ?? ''} onChange={(e) => updateStore(store.id, { priorYearRevenue: Math.abs(parseFloat(e.target.value) || 0) })} placeholder="Нужен для режима НДС в 2026" /><p className="text-[11px] text-stone-500">Используется для определения обязанности по НДС и доступной ставки в 2026 году.</p></div>}
            ...
            {store.regime === 'psn' && <div className="space-y-1.5"><Label className="text-xs">Потенциально возможный доход по патенту, ₽</Label><Input inputMode="numeric" value={store.patentPotentialIncome || ''} onChange={(e) => updateStore(store.id, { patentPotentialIncome: Math.abs(parseFloat(e.target.value) || 0) })} /><p className="text-[11px] text-stone-500">Берётся из строки «Налоговая база» выданного патента и используется для расчёта дополнительного 1% взносов.</p></div>}
