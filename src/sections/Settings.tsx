        ...
        <Card key={store.id} className="bg-white">
          ...
          <CardContent className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={store.legalForm ?? 'ip'} onValueChange={(value) => updateStore(store.id, { legalForm: value as LegalForm })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent><SelectItem value="ip">ИП</SelectItem><SelectItem value="ooo">ООО</SelectItem></SelectContent>
              </Select>
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs">Налоговый режим</Label>
              <Select value={store.regime} onValueChange={(value) => updateStore(store.id, { regime: value as TaxRegime })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>{Object.entries(REGIME_LABELS).map(([id, label]) => <SelectItem key={id} value={id}>{label}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            ...
            {(store.regime === 'usn6' || store.regime === 'usn15') && <div className="space-y-1.5"><Label className="text-xs">Доход за предыдущий год, ₽</Label><Input inputMode="numeric" value={store.priorYearRevenue ?? ''} onChange={(e) => updateStore(store.id, { priorYearRevenue: Math.abs(parseFloat(e.target.value) || 0) })} placeholder="Нужен для режима НДС в 2026" /></div>}
            {(store.regime === 'usn6' || store.regime === 'usn15') && <div className="space-y-1.5"><Label className="text-xs">НДС при УСН, 2026</Label><Select value={store.vatMode ?? 'auto'} onValueChange={(value) => updateStore(store.id, { vatMode: value as VatMode })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="auto">Авто по доходу</SelectItem><SelectItem value="exempt">Освобождение</SelectItem><SelectItem value="vat5">НДС 5%</SelectItem><SelectItem value="vat7">НДС 7%</SelectItem><SelectItem value="vat22">НДС 22% + вычеты</SelectItem></SelectContent></Select></div>}
            {(store.regime === 'usn6' || store.regime === 'usn15' || store.regime === 'psn') && <div className="space-y-1.5"><Label className="text-xs">Страховые взносы за год, ₽</Label><Input inputMode="numeric" value={store.insurancePremiums || ''} onChange={(e) => updateStore(store.id, { insurancePremiums: Math.abs(parseFloat(e.target.value) || 0) })} placeholder="Пусто = авторасчёт для ИП без работников" /></div>}
            ...
            {store.regime === 'psn' && <div className="space-y-1.5"><Label className="text-xs">Потенциально возможный доход по патенту, ₽</Label><Input inputMode="numeric" value={store.patentPotentialIncome || ''} onChange={(e) => updateStore(store.id, { patentPotentialIncome: Math.abs(parseFloat(e.target.value) || 0) })} /></div>}
            {store.regime === 'npd' && <div className="space-y-1.5"><Label className="text-xs">Ставка НПД</Label><Select value={String(store.npdRate ?? 6)} onValueChange={(value) => updateStore(store.id, { npdRate: Number(value) })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="4">4% (физлица)</SelectItem><SelectItem value="6">6% (организации / ИП)</SelectItem></SelectContent></Select></div>}
            {(store.regime === 'usn6' || store.regime === 'usn15' || store.regime === 'psn') && <div className="flex items-center gap-3 pt-5"><Switch checked={store.hasEmployees} onCheckedChange={(value) => updateStore(store.id, { hasEmployees: value })} /><Label className="text-xs">Есть сотрудники</Label></div>}
          </CardContent>
        </Card>
      ))}
      ...
      <Card className="bg-white">
        ...
        <CardContent className="space-y-3">
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="outline" onClick={exportJson}><Download className="mr-2 h-4 w-4" /> Скачать безопасную копию</Button>
          ...
          <p className="text-xs text-stone-500">Оба экспорта исключают API-ключи. После восстановления резервной копии подключения к маркетплейсам настраиваются заново.</p>
        </CardContent>
      </Card>
    </div>
  )
