...
import { checkOrganizationCompleteness, checkStoreCompleteness, type CompletenessLevel } from '@/lib/dataCompleteness'
  ...
  return (
    <div className="space-y-6">
      ...
      {is2026 && organizations.map((organization) => {
        const summary = calcOrganizationTax(organization.id, state.stores, state.operations, periodFrom, periodTo, allocation)
        if (!summary.lines.length) return null
        const completeness = checkOrganizationCompleteness(state, organization.id, periodFrom, periodTo)
        return (
          <Card key={organization.id} className={completeness.finalCalculationAllowed ? 'border-emerald-200 bg-emerald-50/30' : 'border-amber-200 bg-amber-50/30'}>
            <CardHeader>
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs text-stone-500">Налоговый контур за период {periodFrom}—{periodTo}</p>
                </div>
            ...
            <CardContent className="space-y-4">
              {!completeness.finalCalculationAllowed && (
                <Alert className="border-amber-300 bg-amber-50">
                  ...
                  <AlertDescription className="text-xs text-amber-950">
                    <b>Окончательный налог не подтверждён.</b> В расчёте есть неполные исходные данные. Суммы ниже можно использовать только как предварительную оценку.
                    <div className="mt-2 space-y-1">
                      {completeness.issues.slice(0, 8).map((issue) => <div key={issue}>• {issue}</div>)}
                      {completeness.issues.length > 8 && <div>• И ещё {completeness.issues.length - 8} замечаний</div>}
                    </div>
                  </AlertDescription>
                </Alert>
              )}
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
                ...
                <Stat label={completeness.finalCalculationAllowed ? 'Использовано для вычета' : 'Вычет — предварительно'} value={fmtMoney(summary.insuranceUsed)} accent />
              </div>

              <div className={`rounded-xl border bg-white p-4 text-sm ${completeness.finalCalculationAllowed ? 'border-emerald-200' : 'border-amber-200'}`}>
                <div className={completeness.finalCalculationAllowed ? 'font-medium text-emerald-900' : 'font-medium text-amber-900'}>
                  {completeness.finalCalculationAllowed ? 'Итог по ИП' : 'Предварительный итог по ИП'}
                </div>
              ...
              <div className="rounded-xl border border-stone-200 bg-white p-4">
                ...
                <div className="grid gap-2 md:grid-cols-2">
                  {completeness.sources.map((source) => (
                    <div key={source.id} className="rounded-lg border border-stone-100 p-3 text-xs">
                      <div className="flex items-center justify-between gap-2">
                        <span className="font-medium">{source.label}</span>
                        <DataStatus level={source.level} compact />
                      </div>
                      <div className="mt-1 text-stone-500">{source.message} · операций: {source.operations}</div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <Card key={store.id} className="bg-white">
            <CardHeader>
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2"><span className="text-sm text-stone-500">{calc.regimeLabel}</span><DataStatus level={completeness.level} compact /></div>
              </div>
            </CardHeader>
            <CardContent className="space-y-5">
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
                ...
                <Stat label={completeness.finalCalculationAllowed ? 'Налог режима' : 'Налог — предварительно'} value={fmtMoney(calc.taxDue)} />
                <Stat label={completeness.finalCalculationAllowed ? 'Всего с НДС' : 'Всего — предварительно'} value={fmtMoney(calc.totalTaxDue)} accent />
              </div>
              ...
              {calc.insurance.total > 0 && (store.regime === 'usn6' || store.regime === 'usn15' || store.regime === 'psn') && (
                <div className="rounded-xl border border-stone-200 bg-stone-50 p-4 text-sm">
                  ...
                  <div className="mt-1 text-stone-700">На уровне точки: {fmtMoney(calc.insurance.total)}. Итоговый вычет ИП показывается выше без дублирования фиксированной части.</div>
              ...
              {(store.regime === 'usn6' || store.regime === 'usn15') && (
                <div>
                  <div className="mb-2 text-sm font-medium">Авансовые платежи {year} {completeness.finalCalculationAllowed ? '' : '— предварительно'}</div>
                  <Table>
                    <TableHeader>
                      <TableRow>
                    ...
                    <TableBody>
                      {advances.map((a) => (
                        <TableRow key={a.label}>
                          ...
                          <TableCell className={`text-right font-semibold ${completeness.finalCalculationAllowed ? 'text-red-700' : 'text-amber-700'}`}>{fmtMoney(a.advance)}</TableCell>
...
function DataStatus({ level, compact = false }: { level: CompletenessLevel; compact?: boolean }) {
  if (level === 'complete') return <span className="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-2 py-1 text-[11px] font-medium text-emerald-800"><CheckCircle2 className="h-3 w-3" />{compact ? 'Полно' : 'Данные полные'}</span>
  if (level === 'partial') return <span className="inline-flex items-center gap-1 rounded-full bg-amber-100 px-2 py-1 text-[11px] font-medium text-amber-800"><AlertTriangle className="h-3 w-3" />{compact ? 'Частично' : 'Данные частичные'}</span>
  return <span className="inline-flex items-center gap-1 rounded-full bg-red-100 px-2 py-1 text-[11px] font-medium text-red-800"><AlertTriangle className="h-3 w-3" />{compact ? 'Не хватает' : 'Данных не хватает'}</span>
}
