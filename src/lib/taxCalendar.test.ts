...
const usnStore = (overrides: Partial<Store> = {}): Store => ({
  ...
  // organization.legalForm — хотя правовая форма логически принадлежит
  // организации/ИП, а не отдельному магазину. Похоже на рассинхрон с более
  // новой доменной моделью (Organization.legalForm появился позже). Пока
  // тестируем поведение как есть, но это стоит унифицировать отдельно —
  // см. риск в чекпоинте.
  legalForm: 'ip',
  regime: 'usn6',
  usnIncomeRate: 6,
